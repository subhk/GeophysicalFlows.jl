...
# Imposed condition is the 'vorticity' given by a smooth elliptic vortex.
# This equations are essentially Eq.(1) of the paper.
...
function Vars(::Dev, grid) where Dev
    T = eltype(grid)
    @devzeros Dev T (grid.nx, grid.ny) ξ u v
    @devzeros Dev Complex{T} (grid.nkr, grid.nl) ξh uh vh
    return Vars(ξ, u, v, ξh, uh, vh) 
end
nothing #hide
...
function calcN!( N, sol, t, clock, vars, params, grid )
    ...
    uξ, uξh = vars.u, vars.uh
    vξ, vξh = vars.v, vars.vh

    @. uξ *= vars.ξ
    @. vξ *= vars.ξ 

    # compute the advection term: u*∂x(ξ) + v*∂y(ξ)
    mul!( uξh, grid.rfftplan, uξ )
    mul!( vξh, grid.rfftplan, vξ )

    @. N = - im * grid.kr * uξh - im * grid.l * vξh 
    return nothing
end 
nothing #hide
...
function updatevars!(prob)
    ...
    ldiv!( vars.ξ, grid.rfftplan, deepcopy(vars.ξh) )
...
function set_ξ!(prob, ξ)
    mul!( prob.sol, prob.grid.rfftplan, ξ )
...
### ouput 
base_filename = string( "qg_Held_", nx )
datapath = "./"
plotpath = "./"
...
mp4(anim, "QG_Held1995.mp4", fps=14)