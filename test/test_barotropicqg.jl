...
function test_bqg_stochasticforcingbudgets(; n=256, dt=0.01, L=2π, nu=1e-7, nnu=2, mu=1e-1, message=false)
  ...
  function calcFq!(F, sol, t, cl, v, p, g)
  ...
  sol, v, p, g, eq, cl = prob.sol, prob.vars, prob.params, prob.grid, prob.eqn, prob.clock

  BarotropicQG.set_zeta!(prob, zeros(g.nx, g.ny))
...
function test_bqg_advection(dt, stepper; n=128, L=2π, nu=1e-2, nnu=1, mu=0.0, message=false)
  ...
  BarotropicQG.updatevars!(prob)
  isapprox(prob.vars.q, qf, rtol=rtol_barotropicqg)
end
...
function test_bqg_formstress(dt, stepper; n=128, L=2π, nu=0.0, nnu=1, mu=0.0, message=false)
  ...
  isapprox(N[2][1], 1/4, rtol=rtol_barotropicqg)
end
...
function test_bqg_energyenstrophy()
  ...
  isapprox(energyzeta0, 29.0/9, rtol=rtol_barotropicqg) && isapprox(enstrophyzeta0, 2701.0/162, rtol=rtol_barotropicqg)
end
...
function test_bqg_meanenergyenstrophy()
 ...
 zeta0 = @. -((2k0)^2+(2l0)^2)*sin(2k0*x)*cos(2l0*y) - (k0^2+(3l0)^2)*2sin(k0*x)*cos(3l0*y)
  ...
  (isapprox(energyU, 0.5*U^2, rtol=rtol_barotropicqg) &&
    isapprox(enstrophyU, beta*U, rtol=rtol_barotropicqg) &&
    isapprox(energyzeta0, 29.0/9, rtol=rtol_barotropicqg) &&
    isapprox(enstrophyzeta0, 2701.0/162, rtol=rtol_barotropicqg)
  )
end
